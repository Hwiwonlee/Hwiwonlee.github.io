...
But If someone has question about the `TotalMinionsKilled + TotalJungleMinionsKilled`, I have to explain the reason of the why do this. The reason is that `TotalMinionsKilled` regarded as team's total CS is quite small at the high rank game. Until 10 miniute, About 100 minions are sent to each lane. Including Jungle minions, each team's the maximum value of CS is 350 or higher. 
...
high_diamond_ranked_10min %>% 
  ...
  mutate(name = factor(name, levels = c("blueTotalMinionsKilled", "redTotalMinionsKilled", "blueTotalMinionsKilledNew", "redTotalMinionsKilledNew"))) %>% 
  mutate(index = ifelse(str_detect(name, "New"), "Using the summation", "Using TotalMinionKilled")) %>% 
  ggplot(aes(x = value)) + 
  geom_histogram(aes(fill = factor(name))) + facet_grid(index ~ ., scales = "free") + 
  theme_minimal() + 
  scale_fill_manual(name = "TotalMinionsKilled",
                    labels = c("Blue team", "Red team", "Blue team(new)", "Red team(new)"), 
                    values = c("dodgerblue4", "firebrick4", "dodgerblue1", "firebrick1"))
...
`TotalGold`, `GoldDiff`, `GoldPerMin`, `TotalExperience`, `ExperienceDiff`
